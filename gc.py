...
Switching models
----------------
Two model lines are defined in the CONFIG section below.  Comment out the one
you are NOT using and uncomment the one you want.  Everything else adapts
automatically (filename, params block, etc.).
...
Each record in the output JSON contains:
    ...
    reward_score    — two-factor reward: mean_quality × quantity_bonus
...
# ── Model selector ──────────────────────────────────────────────────────────
# Comment out the model you are NOT using, leave the other uncommented.
...
# Pattern 2 — bare URL in running text (small/large model fallback)
...
def build_prompt(instruction: str, tokenizer) -> str:
    ...
    recall on instruction-tuned models without any fine-tuning.
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "What are the primary colors of light?",
        },
        {
            "role": "assistant",
            "content": (
...
def run_batch(
    ...
    ----------
    model_name:
        HuggingFace model identifier (set via the MODEL_NAME constant above).
    temp:
    ...
    # llm = LLM(model=model_name, dtype="bfloat16", device="cuda", gpu_memory_utilization=0.4)
    # llm = LLM(model=model_name, dtype="bfloat16")
    # llm = LLM(model=model_name, dtype="bfloat16", gpu_memory_utilization=0.3, quantization="gptq")
    # llm = LLM(model=model_name, dtype="bfloat16", gpu_memory_utilization=0.25)
    # llm = LLM(model=model_name, dtype="bfloat16", gpu_memory_utilization=0.25, max_model_len=1024)
    # llm = LLM(model=model_name, dtype="bfloat16", gpu_memory_utilization=0.25, max_model_len=1024)
    llm = LLM(model=model_name, dtype="float16", gpu_memory_utilization=0.15, max_model_len=512)
    print("Engine ready.\n")

    # ── Sampling params ──────────────────────────────────────────────────────
    # LLaMA 3 uses <|eot_id|> as its turn-end token; Mistral uses </s>.
    # We collect both stop IDs so the same SamplingParams works for either
    # model — the one that doesn't exist in the vocabulary is silently ignored
    # by vLLM.
    ...
    # ── Batch inference (all prompts × all N samples in one call) ────────────
